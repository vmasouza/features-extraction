function melfcc = mfcc(s,nC)

    fs = 16000;
    melfcc = extract_mfcc(s, fs, length(s), length(s), nC, 1, 8000)';

end



% Addapted from Dan Ellis's code
function melfcc = extract_mfcc(s, fs, wsize, ssize, numcep, minfreq, maxfreq)
    ...
    pspectrum = abs(specgram(s*32768,2^(ceil(log(wsize)/log(2))),fs,hamming(wsize),wsize - ssize)).^2 + eps;

    aspectrum = audspec(pspectrum, fs, nbands, minfreq, maxfreq, sumpower, bwidth);

    % Convert to cepstra via DCT
    cepstra = spec2cep(aspectrum, numcep, dcttype);

    melfcc = zeros(numcep,1);
    csize = size(cepstra);
    for i = 1 : numcep
        for j = 1: csize(2)
            melfcc(i) = cepstra(i,j);
        end
        melfcc(i) = melfcc(i)/length(cepstra);
    end
    
end

function [aspectrum,wts] = audspec(pspectrum, sr, nfilts, minfreq, maxfreq, sumpower, bwidth)
    %[aspectrum,wts] = audspec(pspectrum, sr, nfilts, minfreq, maxfreq, sumpower, bwidth)
    %
    % perform critical band analysis (see PLP)
    % takes power spectrogram as input

    nfreqs = length(pspectrum);
